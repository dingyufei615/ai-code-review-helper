...
def _load_prompts_if_needed():
    ...
    if _PROMPTS is None:
        try:
            ...
            logger.error(
                f"CRITICAL: Prompt template file not found: {_PROMPT_FILE_PATH}. Prompts will not be available.")
            _PROMPTS = {}  # Fallback to empty dict
        except yaml.YAMLError as e:
            logger.error(
                f"CRITICAL: Error parsing prompt template file {_PROMPT_FILE_PATH}: {e}. Prompts may be incomplete.")
            _PROMPTS = {}  # Fallback
