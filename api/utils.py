...
def parse_single_file_diff_for_codeup(diff_text, file_path, old_file_path=None):
    """
    解析单个文件的 Codeup diff 格式文本，提取变更信息。
    此函数目前与通用解析器相同，但为将来可能的 Codeup 特定调整提供了独立的扩展点。
    返回包含该文件变更详情和上下文的字典。
    """
    file_changes = {
        "path": file_path,
...
def verify_codeup_signature(req, secret):
    """验证 Codeup Webhook 签名 (基于假设的 X-Codeup-Token)"""
    # 假设 Codeup 使用 'X-Codeup-Token' 头和静态令牌进行验证
    codeup_token = req.headers.get('X-Codeup-Token')
    if not codeup_token:
        logger.error("错误: X-Codeup-Token 请求头缺失。请确认为 Codeup 配置的 Webhook 使用了此 Header。")
        return False
    if not secret:
        logger.error("错误: 此项目未配置 Webhook secret。")
        return False
